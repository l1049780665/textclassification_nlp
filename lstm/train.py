...
#这段代码首先加载数据并划分为训练集、验证集和测试集。然后，对文本数据进行预处理，包括标签编码、文本向量化等操作。
#接下来，构建了一个简单的LSTM模型，通过Keras进行模型的编译和训练。训练完成后，保存了模型和对文本进行向量化的Tokenizer对象。
#这段代码主要完成了数据的预处理、LSTM模型的构建和训练，以及模型的保存
